...
<!-- 2. 세번째 row 의 배경색을 빨간색으로 출력 (inline css 속성 사용 x) -->
...
<table>
	...
	<c:choose>
		<c:when test="${vs.count eq 3}"><c:set var="clzAttr" value="red"/><!-- <tr class="red"> --></c:when>
		<c:when test="${vs.last}"><c:set var="clzAttr" value="green"/><!-- <tr class="green"> --></c:when>
		<c:otherwise><c:set var="clzAttr" value="normal"/><!-- <tr> --></c:otherwise>			
	</c:choose>
	<tr class="${clzAttr }">
