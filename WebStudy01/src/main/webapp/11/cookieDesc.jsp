...
<pre>
	...
	사용 단계
		...
		<%
			...
			String koreanValue = URLEncoder.encode("한글값", "UTF-8");	//!! 원래는 500 error 발생
...
// 			response.addCookie(domainCookie);
			...
			longLiveCookie.setPath("/");		// localhost면 어디서든 사용가능
	...
	** 쿠키 속성들
	...
	부가 속성
		domain(host) : 다음번 요청에 포함시켜 재전송할지 여부를 결정하는 조건
				ex) .naver.com(호스트 상위 까지 같으면 재전송), www.naver.com(호스트까지 일치)
		path : 다음번 요청에 포함시켜 재전송할지 여부를 결정하는 조건
			path 설정이 명시되지 않은 경우, 쿠키 생성 경로가 반영됨
		maxAge : 쿠키의 만료 시한
			ex) -1(브라우저가 종료 시 바로 삭제), 0(name, value... 등 모든 속성이 동일한 기존 longliveCookie가 있는 경우 삭제)
		secure...
</pre>
</body>
</html>