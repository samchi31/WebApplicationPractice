...
public class BootstrapPaginationRenderer implements PaginationRenderer {
	...
	public String renderPagination(PagingVO<?> pagingVO) {
		String prevPtrn = "<li class='page-item %s'><a class='page-link paging' %s data-page='%d'>Prev</a></li>";
		...
		html.append(String.format(prevPtrn, disabled?"disabled":"", disabled?"":"href='#'", disabled?1:startPage-1));
		
		for(int page=startPage; page<=endPage; page++) {
			if(page == currentPage) {
				html.append(String.format(currentPtrn, page));
			} else {
		...
		html.append(String.format(nextPtrn, disabled?"disabled":"", disabled?"":"href='#'", disabled?totalPage:endPage+1));
